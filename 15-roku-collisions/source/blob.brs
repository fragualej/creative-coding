function _blob(x, y, size)
    m = {
        radius: size
        pos: vector(x, y)
        vel: vector(randomRange(-10, 10), randomRange(-10, 10))
        mass: size * 0.5
        screen: m.screen
        draw: sub()
            slides = 24
            ...
            for i = 0 to slides - 1
                ...
                m.screen.drawLine(x1, y1, x2, y2, colorUtils_getByName("white"))
            end for
        end sub
        update: sub()
            m.pos.x += m.vel.x
        ...
        bounce: sub(ix, iy, width, height)
            if m.pos.x - m.radius < ix or m.pos.x + m.radius > ix + width then m.vel.x *= -1
            if m.pos.y - m.radius < iy or m.pos.y + m.radius > iy + height then m.vel.y *= -1
        end sub
        collision: sub(other as object)
            ...
            if d < m.radius + other.radius then
                ...
                ' Calculate the new velocities on x and y axis
